...
export const addSaved = async (req, res) => {
    try {
        ...
        if (product.store.ownerId !== userId) {
            ...
            const saverName = `${saver?.firstName ?? ""} ${saver?.lastName ?? ""}`.trim() || "Someone";
            await notifySavedProduct(req.app.get("io"), // socket.io instance
...
export const unsaveProduct = async (req, res) => {
    try {
        ...
        if (savedProduct.product.store.ownerId !== userId) {
            ...
            const unsaverName = `${unsaver?.firstName ?? ""} ${unsaver?.lastName ?? ""}`.trim() || "Someone";
            await notifyUnsavedProduct(req.app.get("io"), // socket.io instance
...
export const removeAllSaved = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ message: "Unauthorized" });
        ...
        const notificationPromises = savedProducts
            ...
            try {
                await notifyUnsavedProduct(req.app.get("io"), savedProduct.product.store.ownerId, userName, savedProduct.product.name);
            }
            catch (error) {
        ...
        Promise.all(notificationPromises).catch(err => console.error("Error sending bulk unsave notifications:", err));
        return res.status(200).json({
            success: true,
